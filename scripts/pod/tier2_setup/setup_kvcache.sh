#!/usr/bin/env bash
# Tier-2 pod setup — KVCache-Factory (SnapKV / H2O; query-aware KV eviction, Llama-3.1-8B). Idempotent.
...
# the HF token must have accepted the Meta license or prefetch SKIPs. Runner: run_kvcompress.py --method h2o|snapkv.
...
step "kvcache ready — smoke: micromamba run -n kvcache python scripts/baselines/tier2/run_kvcompress.py --method h2o --dataset longmemeval --max-examples 5"
