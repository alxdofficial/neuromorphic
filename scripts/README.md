...
active (imports use `src.repr_learning.*`):
...
| -------------- | ------------- |
| `train/`       | `train_repr_qa.py` — the single training harness for every variant + task (the active task is `--task sentence_mae`). |
| `diagnostics/` | Smoke tests, the floor/ceiling band scan, and the hierarchical-learned-vocab debug sweeps. |
| `data_gen/`    | Synthetic data generation for the (dormant, kept) composite-QA + EMAT-bio universe (biographical worldspec, task generators). Imported by `src/repr_learning/data_qa.py`, `data_emat_bio.py`, `emat_bio_templates.py`. |

## diagnostics/ quick reference (the active compression line)
```bash
python scripts/diagnostics/mae_smoke.py            # pre-flight: all variants construct/train/grad-flow on sentence_mae
...
## train/
```bash
python scripts/train/train_repr_qa.py --task sentence_mae --backbone HuggingFaceTB/SmolLM2-135M \
...
- Models (one self-contained folder each): `src/repr_learning/models/`
