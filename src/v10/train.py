"""
Entry point for v10-gnn training.

Usage:
    python -m src.v10.train                        # defaults
    python -m src.v10.train --bs 8 --steps 5000    # override
    python -m src.v10.train --no-memory             # LM-only baseline
"""
...
from .trainer import V10Trainer
from .diagnostics import V10Diagnostics
...
LOG_INTERVAL = 50
SAVE_INTERVAL = 5000
SAVE_DIR = "outputs/v10"


def parse_args():
    p = argparse.ArgumentParser(description="v10-gnn training")
    ...
    p.add_argument("--d-inner", type=int, default=None,
                   help="Override d_inner_scan (for param-matched baselines)")
    return p.parse_args()
...
def main():
    ...
    # Config
    config = V10Config.tier_a()
    config.vocab_size = vocab_size
    config.eot_id = eot_id
    if args.d_inner is not None:
        config.d_inner_scan = args.d_inner
    config.validate()

    bs = args.bs
    T = config.T
    print(f"\nConfig: D_scan={config.D_scan}, D_neuron={config.D_neuron}, "
          f"D_dec={config.D_dec}")
    print(f"  Lower scan: L={config.L_scan}, d_inner={config.d_inner_scan}")
    print(f"  Memory: {config.N_neurons} neurons, K={config.K_connections}, "
          f"D_neuron={config.D_neuron}, D_id={config.D_id}")
    print(f"  Decoder: L={config.L_dec}, n_heads={config.n_heads_dec}, "
          f"d_ff={config.d_ff_dec}, W_sliding={config.W_sliding}")
    print(f"  Shared MLPs: H_state={config.H_state}, "
          f"H_msg={config.H_msg}, H_mod={config.H_mod}")
    print(f"  Training: BS={bs}, T={T}, "
          f"mem_lr_scale={config.mem_lr_scale}")

    # Model -- everything in bf16, optimizer keeps f32 master weights
    model = V10Model(config)
    if device.type == "cuda":
    ...
    if args.resume:
        ...
        model.lm.load_state_dict(ckpt["lm_state_dict"], strict=False)
    ...
    dec_params = model.decoder_param_count()
    total_params = model.param_count()
    print(f"\nParameters: {total_params:,} ({total_params/1e6:.1f}M)")
    print(f"  LM (lower scan): {lm_params:,} ({lm_params/1e6:.1f}M)")
    print(f"  Memory graph:    {mem_params:,} ({mem_params/1e6:.1f}M)")
    print(f"  Decoder:         {dec_params:,} ({dec_params/1e6:.1f}M)")
    ...
    optimizer = torch.optim.AdamW([
        ...
        {"params": mem_decay, "weight_decay": WEIGHT_DECAY * 0.1,
         "lr": mem_lr},
        {"params": mem_no_decay, "weight_decay": 0.0, "lr": mem_lr},
        {"params": dec_decay, "weight_decay": WEIGHT_DECAY, "lr": args.lr},
        {"params": dec_no_decay, "weight_decay": 0.0, "lr": args.lr},
    ...
    # Data -- on resume, only need enough batches for remaining steps
    ...
    diag = V10Diagnostics(
        model, save_dir, snapshot_every=args.snapshot_interval)
    saved_checkpoints = []
    ...
    def on_step(metrics):
        ...
        # Auto-plot every 250 steps (async subprocess)
        nonlocal _plot_proc
        if step % 250 == 0 and step > 0:
            if _plot_proc is not None and _plot_proc.poll() is None:
                pass  # previous plot still in progress
            else:
                try:
                    metrics_file.flush()
                    _plot_proc = subprocess.Popen(
                        [sys.executable, "-m", "scripts.plot_training",
                         save_dir],
        ...
        if args.save_interval > 0 and step % args.save_interval == 0:
            _save_checkpoint(model, optimizer, scheduler, config,
                             step, save_dir, saved_checkpoints,
    ...
    _save_checkpoint(model, optimizer, scheduler, config,
                     final_step, save_dir, saved_checkpoints,
                     keep=0)  # keep=0 means don't prune final

    metrics_file.close()

    if all_metrics:
        final = all_metrics[-1]
        print(f"\nDone. Final loss: {final['loss']:.4f}, "
              f"ppl: {final['ppl']:.1f}")
    print(f"Checkpoint: {os.path.join(save_dir, f'v10_step{final_step}.pt')}")


def _save_checkpoint(model, optimizer, scheduler, config, step,
                     save_dir, saved_checkpoints, keep=3):
    """Save a checkpoint with all model components + optimizer state."""
    ckpt_path = os.path.join(save_dir, f"v10_step{step}.pt")
    ckpt = {
    ...
    if keep > 0:
        while len(saved_checkpoints) > keep:
