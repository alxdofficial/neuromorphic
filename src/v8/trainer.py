"""V8/v9.1 Trainer — single backward pass for LM + memory graph.

Each step:
1. Full scan over T tokens (lower scan + PCM + memory graph + upper scan)
2. Single backward on CE + aux_loss (gradients flow through memory graph)
3. Single optimizer step (LM + memory params with separate LR groups)
"""
...
class V8Trainer:
    """Training loop: single optimizer, single backward pass."""
    ...
    def train_chunk(self, batch) -> dict:
        """Process one chunk. Single backward through LM + memory."""
        self.model.train()
        ...
        # Forward (memory graph is on the compute graph)
        with amp_ctx:
        ...
        # Single backward — gradients flow through both LM and memory
        self.optimizer.zero_grad(set_to_none=True)
        total_loss.backward()

        # Clip gradients (LM and memory params separately for monitoring)
    ...
    def train_epoch(self, max_steps: int, step_callback=None) -> list[dict]:
        all_metrics = []
        pbar = tqdm(total=max_steps, desc="v9.1 train", unit="step")
