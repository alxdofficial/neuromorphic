...
baseline: n=3 seeds. Auto-generated by `scripts/diagnostics/slotgraph_metrics.py` — re-run after each fix; compare two designs with `--vs`. Values are mean±std over seeds.

## How to read this report

The model is a pipeline: **WRITE** (the 32 slots come out of the frozen LM) → **SELECT** (content-addressed routing picks each edge's node endpoints → a graph) → **MP-READ** (multi-hop message passing over that graph) → **OUTPUT** (the prepended memory + task perf). Each stage's metrics tell us if that stage is healthy.

Every *selection* is judged under two lenses:
- **decisiveness** — is the choice sharp given ONE input? (↑ good)
- **cross-input diversity** — does the choice CHANGE with the input? (↑ good — this is the topology signal; a selection that's decisive but not diverse is a fixed wiring)

Good-direction symbols: **↑** higher is better · **↓** lower is better · **→0** should be ~0 · **·** context (no fixed target). In compare mode each cell is `A→B Δ` with **✓** (moved the good way) / **✗** (moved the wrong way). The **depth profile** traces effective rank per LM layer (write) then per MP hop (read) — where rank collapses. The **node histogram** shows per-node usage over samples (flat = fixed roles, peaked = a hub). Full definitions are in the glossary at the bottom.
...
| mae loss | ↓ | 6.536±0.008 |
| mae SHUF−REAL | ↑ | 0.442±0.044 |
