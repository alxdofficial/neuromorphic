...
Headline vs **GPT-4o/Qwen-long-context + RAG + MemoryLLM/M+ + EM-LLM**, on **LongMemEval** (+ BEAM for scale, + WikiBigEdit for stability), with **ARC-Encoder + ICAE + xRAG/PISCO + Cartridges** as compression cousins, bracketed by the **full-context oracle** and **no-memory floor** we already run.
