...
def test_write_module_construct():
    ...
    # state_update is nn.GRUCell — replaces the old mutate_mlp + decay_gate.
    # Input is concat[current_state(D), history_attn(D), cross_attn(D), surprise(1)].
    assert isinstance(wm.state_update, torch.nn.GRUCell)
    assert wm.state_update.input_size == cfg.D_concept * 3 + 1
    assert wm.state_update.hidden_size == cfg.D_concept
...
def test_write_module_gru_state_update_bounded():
    """GRUCell's tanh candidate makes state magnitude bounded.

    Repeatedly applying the state update should not grow ||state||
    unboundedly — convex combination of bounded (current) with bounded
    (tanh candidate) is itself bounded. This is the direct fix for the
    Wave 1 step-14564 drift bug.
    """
    cfg = TrajMemConfig.small()
    wm = WriteTrajectoryGenerator(cfg)
    D = cfg.D_concept
    # Start with a unit-norm state, drive with random input 200 times.
    state = torch.nn.functional.normalize(torch.randn(8, D), dim=-1)
    max_norm = state.norm(dim=-1).max().item()
    with torch.no_grad():
        for _ in range(200):
            step_input = torch.randn(8, D * 3 + 1)
            state = wm.state_update(step_input, state)
            max_norm = max(max_norm, state.norm(dim=-1).max().item())
    # GRUCell's hidden state is bounded by tanh => ||state|| <= sqrt(D)
    # (each dim in [-1, 1]). In practice it stays much smaller.
    assert max_norm < (D ** 0.5) + 1e-3, (
        f"GRUCell state grew past tanh bound: max_norm={max_norm:.3f}"
    )
