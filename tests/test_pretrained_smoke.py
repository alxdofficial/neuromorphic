"""Smoke tests for the pretrained-LM + memory model.
...
def test_scale_zero_reproduces_vanilla_llama():
    """With scale pinned to 0, model output == vanilla Llama output exactly."""
    from src.pretrained.config import PretrainedConfig
    from src.pretrained.llm_wrapper import PretrainedLMWithMemory
    ...
    with torch.no_grad():
        model.mem_inject.scale.zero_()

    input_ids = torch.randint(0, 128256, (2, 16))
    with torch.no_grad():
        out_wrapper = model(input_ids).logits
...
def test_scale_nonzero_with_no_memory_is_still_transparent():
    ...
    cfg = PretrainedConfig.llama_1b(llama_dtype="fp32")
    model = PretrainedLMWithMemory(cfg, attach_memory=False)
    model.train(False)
    ...
    # Scale stays at init (2.0). memory is not attached — the model's
...
def test_memory_wired_forward_runs_and_differs_from_vanilla():
    ...
    # model output equals vanilla regardless of memory being attached.
    ...
    assert model._last_mem_pred_loss is not None
    assert torch.isfinite(model._last_mem_pred_loss)
...
def test_memory_wired_backward_reaches_trainable_params():
    ...
    loss = ce + 0.1 * model._last_mem_pred_loss
    loss.backward()

    L = cfg.inject_layer
    mem_inj = model.mem_inject
    ...
    # receive gradient.
    assert model.memory.modulator.tok_proj[0].weight.grad is not None
    assert model.memory.discrete_policy.codebook.grad is not None
    assert model.memory.inject_w.grad is not None

    # Frozen backbone: no grads on Llama's embed / layers[0] / norm.
    assert model.llama.model.embed_tokens.weight.grad is None
    assert model.llama.model.layers[0].self_attn.q_proj.weight.grad is None
    assert model.llama.model.norm.weight.grad is None
    assert model.llama.lm_head.weight.grad is None
...
def test_full_optimizer_step_runs():
    ...
    W_in_before = model.mem_inject.W_in.weight.detach().clone()
    scale_before = model.mem_inject.scale.detach().clone()
    # And one frozen param to check it doesn't move.
    embed_before = model.llama.model.embed_tokens.weight.detach().clone()
    ...
    assert not torch.equal(model.mem_inject.W_in.weight, W_in_before), (
        "W_in did not update after optimizer.step")
    assert not torch.equal(model.mem_inject.scale, scale_before), (
        "scale did not update after optimizer.step")
    assert torch.equal(model.llama.model.embed_tokens.weight, embed_before), (
...
def test_multi_segment_tbptt_carries_memory_state():
    ...
    _ = model(input_ids_1)
    W_after_seg1 = model.memory.W.detach().clone()
    ...
    # not reset to zeros.
    _ = model(input_ids_2)
    W_after_seg2 = model.memory.W.detach()
...
def test_multi_step_training_loop_runs_without_crashing():
    ...
    T = 2 * cfg.memory.modulation_interval
    losses = []
    W_in_snapshot = model.mem_inject.W_in.weight.detach().clone()
...
def test_phase1_on_real_wikitext_stream():
    ...
    end-to-end path — real text → Llama tokenizer → model forward →
    ...
    logs: list = []
    run_phase1(model, opt, data_iter(), steps=3,
               mem_pred_weight=0.1,
...
def test_autoregressive_rollout_runs_and_diverges_across_k():
    ...
    result = autoregressive_rollout(
        model, prefix_ids, gen_length=gen_length, num_rollouts=K,
...
def test_autoregressive_rollout_restores_wrapper_mode():
    ...
    model = PretrainedLMWithMemory(cfg)
    model.train(True)
    model.current_phase = "phase1"

    prefix_ids = torch.randint(0, cfg.vocab_size_lm, (1, 4))
    _ = autoregressive_rollout(
        model, prefix_ids, gen_length=2, num_rollouts=2, seed=0)

    assert model.training is True
    assert model.current_phase == "phase1"
...
def test_phase2_grpo_step_runs_and_flows_gradient_to_modulator():
    ...
    codebook_before = model.memory.discrete_policy.codebook.detach().clone()
    logit_head_before = model.memory.modulator.logit_head.weight.detach().clone()
    ...
    assert not torch.equal(
        model.memory.discrete_policy.codebook, codebook_before)
    assert not torch.equal(
        model.memory.modulator.logit_head.weight, logit_head_before)


@requires_llama
def test_autoregressive_rollout_is_deterministic_under_seed():
    """Two seeded calls to autoregressive_rollout(..., seed=N) on the same
    model state must produce identical token streams and identical
    final logits. Regression guard: the prior design called
    `model.train(True)` during the prefix pass to activate hard
    ...
    r1 = autoregressive_rollout(
        model, prefix, gen_length=4, num_rollouts=2,
...
def test_last_log_pi_sum_cleared_after_phase1_forward():
    ...
    model.train()
    _ = model(ids)
    assert model.memory._last_log_pi_sum is None, (
    ...
    _ = model(ids)
    lp = model.memory._last_log_pi_sum
...
def test_phase1_autoregressive_unroll_backprop_reaches_prefix_fires():
    ...
    tok_proj_before = model.memory.modulator.tok_proj[0].weight.detach().clone()
    cell_emb_before = model.memory.modulator.cell_emb.detach().clone()

    opt = torch.optim.AdamW(
        [p for _, p in model.trainable_parameters()], lr=1e-3)
    ...
    logs = []
    run_phase1_ar(model, opt, data_iter(),
    ...
    # modulator.
    assert not torch.equal(
        model.memory.modulator.tok_proj[0].weight, tok_proj_before), (
        "tok_proj did not update — AR unroll grad did not reach prefix fires")
    assert not torch.equal(
        model.memory.modulator.cell_emb, cell_emb_before), (
...
def test_freeze_codebook_decoder_leaves_modulator_trainable():
    ...
    cfg = PretrainedConfig.llama_1b()
    model = PretrainedLMWithMemory(cfg)
    model.freeze_codebook_decoder()

    assert not model.memory.discrete_policy.codebook.requires_grad
    for p in model.memory.decoder.parameters():
        assert not p.requires_grad
    # Modulator + W_in/W_out/scale still trainable.
    assert model.memory.modulator.logit_head.weight.requires_grad
    assert model.memory.modulator.tok_proj[0].weight.requires_grad
    assert model.mem_inject.W_in.weight.requires_grad
    assert model.mem_inject.W_out.weight.requires_grad
    assert model.mem_inject.scale.requires_grad
    # Memory dynamics (inject_w, msg MLPs, gamma logits) still trainable.
    assert model.memory.inject_w.requires_grad
    assert model.memory.msg_w1.requires_grad
...
def test_freeze_all_but_logit_head_leaves_only_logit_head():
    ...
    cfg = PretrainedConfig.llama_1b()
    model = PretrainedLMWithMemory(cfg)
    model.freeze_all_but_logit_head()

    trainable = {n for n, p in model.trainable_parameters()}
    assert trainable == {
...
def _trainable_param_groups(model) -> dict:
    """Enumerate every trainable parameter path in the model, grouped
    by what they test. Any dead group is a potential training bug."""
    L = model.config.inject_layer
    groups: dict[str, list[tuple[str, torch.Tensor]]] = {
        "inject_projections": [
            (f"mem_inject.W_in", model.mem_inject.W_in.weight),
            (f"mem_inject.W_out", model.mem_inject.W_out.weight),
            (f"mem_inject.scale", model.mem_inject.scale),
        ],
        "modulator_encoder": [
            ("tok_proj[0]", model.memory.modulator.tok_proj[0].weight),
            ("tok_proj[-1]", model.memory.modulator.tok_proj[-1].weight),
            ("h_proj", model.memory.modulator.h_proj.weight),
            ("msg_emit_proj", model.memory.modulator.msg_emit_proj.weight),
            ("msg_recv_proj", model.memory.modulator.msg_recv_proj.weight),
            ("role_emb", model.memory.modulator.role_emb.weight),
            ("cell_emb", model.memory.modulator.cell_emb),
            ("edge_bias_mlp[0]", model.memory.modulator.edge_bias_mlp[0].weight),
        ],
        "modulator_attention": [
            (f"layers[{i}].qkv", model.memory.modulator.layers[i].qkv.weight)
            for i in range(len(model.memory.modulator.layers))
        ] + [
            (f"layers[{i}].out_proj", model.memory.modulator.layers[i].out_proj.weight)
            for i in range(len(model.memory.modulator.layers))
        ],
        "modulator_output": [
            ("pool_norm", model.memory.modulator.pool_norm.weight),
            ("logit_head", model.memory.modulator.logit_head.weight),
        ],
        "codebook": [("codebook", model.memory.discrete_policy.codebook)],
        "decoder": [
            ("decoder.mlp[0]", model.memory.decoder.mlp[0].weight),
            ("decoder.mlp[-1]", model.memory.decoder.mlp[-1].weight),
        ],
        "memory_dynamics": [
            ("inject_w", model.memory.inject_w),
            ("msg_w1", model.memory.msg_w1),
            ("msg_w2", model.memory.msg_w2),
            ("W_decay_logit", model.memory.W_decay_logit),
            ("decay_gamma_logit", model.memory.decay_gamma_logit),
            ("hebbian_decay_logit", model.memory.hebbian_decay_logit),
            ("neuron_id", model.memory.neuron_id),
        ],
    }
    return groups
...
def _warm_up_decoder(model, opt, ids, tgt):
...
def test_gradient_flow_phase1_parallel_covers_all_trainables():
    ...
    opt = torch.optim.AdamW(
        [p for _, p in model.trainable_parameters()], lr=1e-3)
    _warm_up_decoder(model, opt, ids, tgt)
    ...
    groups = _trainable_param_groups(model)
...
def test_gradient_flow_phase1_ar_covers_all_trainables():
    ...
    the SAME full set of trainables through the preserve_autograd_graph
    ...
    _warm_up_decoder(model, opt, warm_ids, warm_tgt)

    model.begin_segment(bs=1)
    with model.preserve_autograd_graph():
        out = model(prefix, use_cache=True)
        past = out.past_key_values
        last = out.logits[:, -1]
        ce_terms = [torch.nn.functional.cross_entropy(last.float(), cont[:, 0])]
        for i in range(T_cont - 1):
            out_i = model(cont[:, i:i + 1], past_key_values=past, use_cache=True)
...
def test_gradient_flow_phase2_grpo_reaches_modulator_and_recurrent():
    ...
    _ = grpo_step(
        model, opt, prefix_ids=prefix, reference_cont=ref,
        num_rollouts=K, gen_length=gen_length,
    ...
    out = model(prefix_rep, use_cache=True)
    log_pi_sum = model.memory._last_log_pi_sum
    ...
    assert model.mem_inject.W_in.weight.grad is not None, (
        "W_in should receive recurrent gradient in phase 2")
    assert model.mem_inject.W_in.weight.grad.abs().sum() > 0

    # W_out and scale should NOT have gradient — they only feed LM logits,
    # and the phase-2 loss doesn't depend on LM logits.
    W_out_g = model.mem_inject.W_out.weight.grad
    scale_g = model.mem_inject.scale.grad
